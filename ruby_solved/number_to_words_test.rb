...
class SayInEnglishTest < MiniTest::Test
  ...
  def test_157_thousand_234
    assert_equal 'one hundred and fifty-seven thousand two hundred and thirty-four', Say.new(157_234).in_english
  end
  ...
  def test_1_million_and_some_crumbs
    assert_equal 'one million and two', Say.new(10**6 + 2).in_english
  end
  ...
  def test_1_billion_and_some_crumbs
    assert_equal 'one billion and two', Say.new(10**9 + 2).in_english
  end
